<template>
  <div>
    ...
    <div class="alert alert-danger" v-if="error"><strong>An error has occurred!</strong> The session could not be invalidated.</div>
      ...
      <table class="table table-striped">
        ...
        <tbody>
          <tr v-for="session in sessions" :key="session.ipAddress">
            ...
            <td>
              <button type="submit" class="btn btn-primary" v-on:click="invalidate(session.series)">Invalidate</button>
            </td>
          </tr>
        </tbody>
      </table>
